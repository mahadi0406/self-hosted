...
import { Eye, EyeOff, Loader2, MessageSquare, Zap, Shield, BarChart3, Send, Users, Clock, Globe } from "lucide-react"
...
export default function Login() {
    const { appName, siteLogo, loginPage } = usePage().props
    ...
    const features = loginPage?.features?.length > 0 ? loginPage.features : defaultFeatures
            ...
            <div className="flex items-center gap-3">
                {siteLogo ? (
                    <img
                        src={siteLogo}
                        alt={appName}
                        className={`${s.container} rounded-xl object-contain`}
                    />
                ) : (
                    <div className={`${s.container} rounded-xl bg-emerald-500 flex items-center justify-center shadow-lg shadow-emerald-500/30`}>
                        <MessageSquare className={`${s.icon} text-white`} strokeWidth={2.5} />
                    </div>
                )}
                <span className={`${s.text} font-bold tracking-tight`}>{appName}</span>
            </div>
        )
    }
        ...
        <div className="min-h-screen flex bg-zinc-950">
                    ...
                    <div className="space-y-4">
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20">
                            ...
                            <span className="text-xs font-medium text-emerald-400 tracking-wider uppercase">
                                {loginPage?.badgeText || 'AI-Powered Messaging'}
                            </span>
                        </div>
                        <h1 className="text-5xl font-bold text-white leading-[1.1] tracking-tight">
                            {loginPage?.headline || 'Reach thousands'}<br />
                            <span className="text-emerald-400">{loginPage?.headlineHighlight || 'in seconds.'}</span>
                        </h1>
                        <p className="text-zinc-400 text-lg leading-relaxed max-w-sm">
                            {loginPage?.description || 'The complete WhatsApp & Telegram broadcast platform with AI-driven campaign intelligence.'}
                        </p>
                    </div>
                        ...
                        <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-500/8 border border-amber-500/20">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-zinc-400">
                                    Email: <span className="text-zinc-200 font-mono">{loginPage.demo.email}</span>
                                    <span className="mx-2 text-zinc-600">·</span>
                                    Password: <span className="text-zinc-200 font-mono">{loginPage.demo.password}</span>
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div className="flex-1 flex flex-col justify-center items-center px-6 py-12 bg-white relative">
                ...
                <div className={`w-full max-w-sm transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                    ...
                    {/* Mobile logo */}
                    <div className="flex lg:hidden mb-10 text-zinc-900">
                        <Logo size="small" />
                    </div>
                    ...
                    {loginPage?.demo?.enabled && (
                        <div className="lg:hidden mt-6 p-4 rounded-lg bg-amber-50 border border-amber-200">
                            ...
                            <p className="text-xs text-zinc-600">
                                <span className="font-mono text-zinc-800">{loginPage.demo.email}</span>
                                <span className="mx-2">·</span>
                                <span className="font-mono text-zinc-800">{loginPage.demo.password}</span>
                            </p>
                        </div>
                    )}
